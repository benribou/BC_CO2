...
export default function App() {
  ...
  const { energyJoules: frontCumJ, co2Grams: frontCumCO2 } =
    useCarbonMeter(500);
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      const temps: TempPoint[] = json.result.map((temp: number, i: number) => ({
        minute: i,
        temperature: temp,
      }));
      console.log("Formatted data:", temps);
      setData(temps);
      ...
      >
        ...
        {backendMetrics && (
          ...
          >
            ...
            <p>
              🖥️ CPU Usage :{" "}
              <strong>{backendMetrics.cpu_usage_kWh.toFixed(6)} kWh</strong>
            </p>
            <p>
              🎮 GPU Usage :{" "}
              <strong>{backendMetrics.gpu_usage_kWh.toFixed(6)} kWh</strong>
            </p>
            <p>
              🧠 RAM Energy :{" "}
              <strong>{backendMetrics.ram_energy_kWh.toFixed(6)} kWh</strong>
            </p>
            <p>
              ⚡ Énergie consommée :{" "}
              <strong>{backendMetrics.energy_consumed.toFixed(6)} kWh</strong>
            </p>
            <p>
              ⏱️ Exec Time :{" "}
              <strong>
                {backendMetrics.exec_time_in_seconds.toFixed(3)} s
              </strong>
            </p>
          </div>
        )}
